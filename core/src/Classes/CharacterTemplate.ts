import Item from '../Items/Item'
import TemplateAbility from '../Types/TemplateAbility'

export default class characterTemplate {
    stats: any
    public name: string = ''
    stat_count: number = 3
    abilities: TemplateAbility[] = []
    item: Item[] = []
    max_items: number = 2
    stats_description: any

    constructor() {
        this.setTemplate()
    }

    setTemplate(class_name: string = 'swordman'): void {
        if (class_name === 'swordman') {
            this.name = 'swordman'
            this.stats = {

            }
            this.stats_description = {
                
            }
            this.abilities = [
                {
                    id: 1,
                    name: 'swing',
                    type: 1,
                    selected: true,
                    desc: 'Cuts nearby enemies.',
                },
                {
                    id: 2,
                    name: 'weapon throw',
                    type: 1,
                    selected: false,
                    desc: 'Throws a copy of your weapon.',
                },
                {
                    id: 10,
                    name: 'dash',
                    type: 1,
                    selected: false,
                    desc: 'Dashes, damaging enemies. The total distance is increased by your attack speed.',
                },
                {
                    id: 3,
                    name: 'jump',
                    type: 2,
                    selected: true,
                    desc: "You jump. There's a minimum and maximum range. Upon landing, you deal damage to units. While airborne, you're immune to ground-based effects.",
                },
                {
                    id: 4,
                    name: 'charge',
                    type: 2,
                    selected: false,
                    desc: 'You dash forward, stunning everyone in your path.',
                },
                {
                    id: 11,
                    name: 'metal thorns',
                    type: 2,
                    selected: false,
                    desc: 'Deals damage over time around you. The chance to deal damage depends on your armor level. The hit rate depends on your attack speed.',
                },
                {
                    id: 5,
                    name: 'whirlwind',
                    type: 3,
                    selected: true,
                    desc: 'You spin your sword, dealing damage to everyone around you. High courage grants additional spins.',
                },
                {
                    id: 6,
                    name: 'quake',
                    type: 3,
                    selected: false,
                    desc: 'Creates waves that rip apart enemies. Each new wave has a larger radius, and the number of waves depends on your health. The first wave deals damage to you and applies weakness.',
                },
                {
                    id: 6,
                    name: 'heaven wrath',
                    type: 3,
                    selected: false,
                    desc: "Gain an 8-second buff that deals damage to enemies if you've struck within the last 1.5 seconds. The frequency of the strikes is based on your attack speed, and the duration increases with your courage.",
                },
                {
                    id: 9,
                    name: 'spectral swords',
                    type: 3,
                    selected: false,
                    desc: 'Summons 5 swords that fight by your side and inherit your attack speed.',
                },
                {
                    id: 7,
                    name: 'cursed weapon',
                    type: 4,
                    selected: true,
                    desc: 'Your weapon gains a cursed power, increasing attack speed and attack range for 12 seconds. After this time, you will take damage. High courage grants a chance to avoid damage.',
                },
                {
                    id: 8,
                    name: 'commands',
                    type: 4,
                    selected: false,
                    desc: 'Grants nearby allies speed and armor for 12 seconds. The effect increases with courage.',
                },
                {
                    id: 8,
                    name: 'inner power',
                    type: 5,
                    selected: true,
                    desc: "When you're almost dead, you gain a powerful buff that helps you survive. It has a long cooldown.",
                },
                {
                    id: 8,
                    name: 'heaven intervention',
                    type: 5,
                    selected: false,
                    desc: 'When you take damage, there is a chance that the heavens will help you.',
                },
            ]
        } else if (class_name === 'flyer') {
            this.name = 'flyer'
            this.stats = {
                
            }
            this.stats_description = {
                
            }
            this.abilities = [
                {
                    id: 1,
                    name: 'fireball',
                    type: 1,
                    selected: true,
                    desc: 'Creates a ball of flame.',
                },
                {
                    id: 2,
                    name: 'frost sphere',
                    type: 1,
                    selected: false,
                    desc: 'Creates a sphere of ice.',
                },
                {
                    id: 9,
                    name: 'lightning bolt',
                    type: 1,
                    selected: false,
                    desc: 'Creates a charge of electricity, damaging one target and shocking nearby targets.',
                },
                {
                    id: 3,
                    name: 'forked lightning',
                    type: 2,
                    selected: true,
                    desc: 'Creates a forked electrical charge. When dealing damage, it has a chance to branch out, creating two new ones.',
                },
                {
                    id: 4,
                    name: 'flamewall',
                    type: 2,
                    selected: false,
                    desc: 'Creates a ring of fire.',
                },
                {
                    id: 5,
                    name: 'light beacon',
                    type: 3,
                    selected: true,
                    desc: 'You fly up and share your vision; you also create electric charges.',
                },
                {
                    id: 6,
                    name: 'frost nova',
                    type: 3,
                    selected: false,
                    desc: 'Creates a circle of frost. Enemies in close range will be shattered, and others will be frozen.',
                },
                {
                    id: 9,
                    name: 'sparks',
                    type: 3,
                    selected: false,
                    desc: 'Creates beams of electricity that radiate out from you and periodically change direction.',
                },
                {
                    id: 7,
                    name: 'teleportation',
                    type: 4,
                    selected: true,
                    desc: 'Teleports you to a certain place.',
                },
                {
                    id: 8,
                    name: 'static field',
                    type: 4,
                    selected: false,
                    desc: 'Units and projectiles cannot move in a static field.',
                },
                {
                    id: 8,
                    name: 'disintegration',
                    type: 5,
                    selected: true,
                    desc: 'Your hits have a chance to apply fragility to enemies.',
                },
                {
                    id: 8,
                    name: 'accumulation',
                    type: 5,
                    selected: false,
                    desc: 'When you cast 10 spells, release magic stars equal to your courage.',
                },
            ]
        } else if (class_name === 'cultist') {
            this.name = 'cultist'
            this.stats = {
                
            }
            this.stats_description = {
                
            }
            this.abilities = [
                {
                    id: 1,
                    name: 'slam',
                    type: 1,
                    selected: true,
                    desc: 'Slam the ground and deal damage to nearby targets.',
                },
                {
                    id: 2,
                    name: 'rune',
                    type: 1,
                    selected: false,
                    desc: 'Create a explosive rune.',
                },
                {
                    id: 10,
                    name: 'soulrender',
                    type: 1,
                    selected: false,
                    desc: 'Tears the enemy, granting a soul charge. Each soul charge increases cast speed. If you have more than one soul charge, there is a chance to tear the enemy apart and lose all soul charges.',
                },
                {
                    id: 3,
                    name: 'shield bash',
                    type: 2,
                    selected: true,
                    desc: 'Shield attack. Damages nearby targets and stuns distant ones.',
                },
                {
                    id: 4,
                    name: 'grim pile',
                    type: 2,
                    selected: false,
                    desc: 'Creates a pile of bones which periodically increases armour rate and movement speed for you and your teammates.',
                },
                {
                    id: 5,
                    name: 'unleashing pain',
                    type: 3,
                    selected: true,
                    desc: 'Summons ghost warriors that hit enemies.',
                },
                {
                    id: 6,
                    name: 'pile of thorns',
                    type: 3,
                    selected: false,
                    desc: 'Creates a pile of bones which periodically damages enemies around it.',
                },
                {
                    id: 9,
                    name: 'wandering evil',
                    type: 3,
                    selected: false,
                    desc: 'Summons an evil that enters a being and, after some time, tears it apart.',
                },
                {
                    id: 7,
                    name: 'shatter-shell',
                    type: 4,
                    selected: true,
                    desc: 'Tears apart nearby enemies without a soul, releasing bones that damage enemies.',
                },
                {
                    id: 8,
                    name: 'ghost form',
                    type: 4,
                    selected: false,
                    desc: 'Become a ghost, becoming immune to damage and gaining phasing.',
                },
                {
                    id: 8,
                    name: 'death aura',
                    type: 5,
                    selected: true,
                    desc: 'Every 4 seconds you release a wave that can damage enemies, the chance depends on your courage.',
                },
                {
                    id: 8,
                    name: 'soul attractor',
                    type: 5,
                    selected: false,
                    desc: 'Every 4 seconds you release a wave that has a chance to rip out the soul of an enemy, the chance depends on your life',
                },
            ]
        }
    }

    increseStat(stat: string): void {
        if (this.stat_count > 0) {
            this.stats[stat]++
            this.stat_count--
        }
    }

    decreaseStat(stat: string): void {
        if (this.stats[stat] > 0) {
            this.stats[stat]--
            this.stat_count++
        }
    }
}
